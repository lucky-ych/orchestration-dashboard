...
try { endpoint = localStorage.getItem('workflow-dashboard-endpoint') || ''; } catch {}
function el(tag, text, className) { const node = document.createElement(tag); if (text !== undefined) node.textContent = text; if (className) node.className = className; return node; }
function validEndpoint(value) { const u = new URL(value); if (u.username || u.password || u.search || u.hash || !u.hostname || !(u.protocol === 'https:' || (u.protocol === 'http:' && ['localhost','127.0.0.1'].includes(u.hostname)))) throw new Error('HTTPS 주소를 입력하세요. 인증 정보·쿼리·해시는 사용할 수 없습니다.'); if(u.pathname==='/' || !u.pathname)u.pathname='/v1/status'; return u.href; }
...
function render() {
  ...
  $('last-update').textContent=`${failed?'마지막 성공':'조회'} ${age(data.generated_at)}${paused?' · 자동 갱신 정지':''}${failed?' · 현재 상태 미확인':''}`;
  ...
  for (const h of data.hosts) {
    ...
    head.append(name,pill(h.heartbeat_status==='fresh'?'하트비트 정상':h.heartbeat_status==='stale'?'하트비트 오래됨':'하트비트 미확인',h.heartbeat_status!=='fresh'));card.append(head);
    ...
    const foot=el('div',undefined,'host-foot'), detail=el('button','기기 상세 ↗');detail.addEventListener('click',()=>showHost(h));foot.append(el('span',`활성 작업 ${h.active_jobs ?? '미확인'} · 예약 ${h.reservations.complete?h.reservations.count:'미확인'}건`),detail);card.append(foot);$('hosts').append(card);
  }
...
function renderCI() { $('ci').replaceChildren();for(const r of data.repositories)for(const c of r.ci){const row=el('div',undefined,'ci-row'),title=el('div');title.append(github(r.repository,'pull',c.pr,`${r.repository} / PR #${c.pr||'—'}`),el('small',c.context||'검증 컨텍스트 미확인'));row.append(title,pill(labels[c.result]||labels[c.state]||'미확인',['failure','error'].includes(c.result)||c.state==='INTERRUPTED'),el('code',c.head?c.head.slice(0,12):'SHA 미확인'),el('time',age(c.updated)));$('ci').append(row);}if(!$('ci').children.length)$('ci').append(el('div',data.availability==='available'?'저장된 CI 기록이 없습니다.':'CI 기록을 확인할 수 없습니다. 저장소 연결 상태를 확인하세요.','empty'));}
function showHost(h){const root=$('host-detail');root.replaceChildren(el('p','HOST DETAIL','eyebrow'),el('h2',h.name));const dl=el('dl');for(const [k,v] of [['워커 ID',h.id],['설정된 역할',h.roles.join(' · ')||'미설정'],['저장소',h.repositories.join(', ')||'없음'],['하트비트',`${h.heartbeat_status} · ${age(h.heartbeat_at)}`],['자원 측정',`${h.metrics_status} · ${age(h.metrics_at)}`],['예약 메모리',`${num(h.reservations.memory_gb)} GiB`],['예약 디스크',`${num(h.reservations.disk_gb)} GiB`],['예약 CPU',`${num(h.reservations.cpu_cores)} cores`],['예약 집계',h.reservations.complete?'모든 설정 저장소 확인됨':'일부 비용 또는 저장소 미확인']])dl.append(el('dt',k),el('dd',v));root.append(dl,el('p','중단된 소유자의 예약도 포함합니다. 기기의 데몬 실행 여부와 활성 작업 수는 별개입니다.'));$('host-dialog').showModal();}
